...
def get_users(current_user: str = Depends(get_current_client), db: Session = Depends(get_db)):
    ...
    users_json = [user_to_dict(user) for user in users]
...
def update_user(username: str, updated_user: UserCreate, current_user: str = Depends(get_current_client), db: Session = Depends(get_db)):
    ...
    if redis_client.exists(cache_key):
        cached_user = redis_client.get(cache_key)
        new_user_check = json.loads(cached_user)
    else:
        new_user_check = db.query(DBUser).filter(DBUser.username == updated_user.username).first()

    if updated_user.username != username and new_user_check:
